...
Character::Character(const string& n, int hp, int rock, int paper, int scissors, string type)
    : name(n),
    maxHp(hp), currentHp(hp), // currentHp is battle-specific, maxHp reflects defined HP
    baseRockDamage(rock), basePaperDamage(paper), baseScissorsDamage(scissors), // Initialize current battle stats
    originalRockDamage(rock), originalPaperDamage(paper), originalScissorsDamage(scissors), originalMaxHp(hp), // Initialize original stats
    bonusDamageNextAttack(0), characterType(std::move(type)) {
}
...
Character::Character(const string& n, int hp, int rock, int paper, int scissors, vector<Passive> p, string type)
    ...
    originalRockDamage(rock), originalPaperDamage(paper), originalScissorsDamage(scissors), originalMaxHp(hp),
    bonusDamageNextAttack(0), passives(std::move(p)), characterType(std::move(type)) {
}
...
// Getters for defined original stats
int Character::getOriginalMaxHp() const { return originalMaxHp; }
...
void Character::resetStatsForNewBattle() {
    currentHp = originalMaxHp; // Reset current HP to the defined original max HP
    maxHp = originalMaxHp;     // Ensure maxHp reflects the original definition
    ...
    for (auto& p : passives) {
        p.triggeredThisBattle = false; // Reset for once-per-battle passives
    }
...
string Character::getShortDescription() const {
    // Uses original stats for consistent display of the character's definition
    return name + " (" + to_string(originalMaxHp) + " HP, R:" + to_string(originalRockDamage)
        + " P:" + to_string(originalPaperDamage) + " S:" + to_string(originalScissorsDamage) + ")";
}

string Character::getFullDescription() const {
    stringstream ss;
    ss << getShortDescription(); // Uses original stats
    if (!passives.empty()) {
        ss << "\n  Passives:";
...
void Character::checkAndApplyPassives(PassiveTrigger triggerType, Character& self, Character& opponent, int move, bool didWin) {
    // Key part for ON_HP_BELOW_PERCENT:
    if (triggerType == PassiveTrigger::ON_HP_BELOW_PERCENT) {
        for (auto& p : passives) {
            ...
            if (p.trigger == PassiveTrigger::ON_HP_BELOW_PERCENT) {
                int hpPercent = (originalMaxHp > 0) ? (static_cast<double>(currentHp) / originalMaxHp * 100) : 0; // Use originalMaxHp for percent calc
                if (currentHp > 0 && hpPercent <= p.threshold) {
                    ...
                    case PassiveEffect::INCREASE_ROCK_DMG_PERM: self.increaseBaseRockDamage(p.value); cout << "  Rock damage (this battle) increased by " << p.value << ".\n"; break;
                    case PassiveEffect::INCREASE_PAPER_DMG_PERM: self.increaseBasePaperDamage(p.value); cout << "  Paper damage (this battle) increased by " << p.value << ".\n"; break;
                    case PassiveEffect::INCREASE_SCISSORS_DMG_PERM: self.increaseBaseScissorsDamage(p.value); cout << "  Scissors damage (this battle) increased by " << p.value << ".\n"; break;
    ...
    for (auto& p : passives) {
        ...
        // ... (switch case for triggerMatches)
        switch (triggerType) {
        ...
        if (triggerMatches) {
            ...
            cout << self.getName() << "'s passive triggered (" << p.getDescription() << ")!\n";
            p.triggeredThisTurn = true;
            // Apply effect (same logic as before)
            switch (p.effect) {
            case PassiveEffect::HEAL_SELF_FLAT: self.heal(p.value); cout << "  Healed " << p.value << " HP.\n"; break;
            case PassiveEffect::DAMAGE_OPPONENT_FLAT: opponent.takeDamage(p.value); cout << "  Dealt " << p.value << " damage to " << opponent.getName() << ".\n"; break;
            case PassiveEffect::INCREASE_NEXT_ATTACK_FLAT: self.addBonusDamageNextAttack(p.value); cout << "  Next attack + " << p.value << " damage.\n"; break;
            ...
            case PassiveEffect::INCREASE_PAPER_DMG_PERM: self.increaseBasePaperDamage(p.value); cout << "  Paper damage (this battle) increased by " << p.value << ".\n"; break;
            case PassiveEffect::INCREASE_SCISSORS_DMG_PERM: self.increaseBaseScissorsDamage(p.value); cout << "  Scissors damage (this battle) increased by " << p.value << ".\n"; break;
            case PassiveEffect::HEAL_SELF_PERCENT_CURRENT: { /* ... */ } break;
            case PassiveEffect::DAMAGE_OPPONENT_PERCENT_CURRENT: { /* ... */ } break;
            default: break;
            }
            if (opponent.isDefeated()) { cout << opponent.getName() << " was defeated by the passive effect!\n"; }
            if (self.isDefeated()) { cout << self.getName() << " was defeated by their own passive effect!?\n"; }
